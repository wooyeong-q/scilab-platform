...
export type Submission = {
  id: string; title: string; author: string; url: string; category: string; grade: string;
  summary: string; tags: string[]; status: 'pending' | 'approved' | 'rejected'; createdAt: string;
};
...
export async function ensureDatabase() {
  ...
  await sql`
    CREATE TABLE IF NOT EXISTS programs (
      id TEXT PRIMARY KEY, title TEXT NOT NULL, summary TEXT NOT NULL, description TEXT NOT NULL,
      category TEXT NOT NULL, grade TEXT NOT NULL, tags JSONB NOT NULL DEFAULT '[]'::jsonb,
      icon TEXT NOT NULL DEFAULT '🧪', url TEXT NOT NULL, author TEXT NOT NULL,
  ...
  await sql`
    CREATE TABLE IF NOT EXISTS submissions (
      ...
      category TEXT NOT NULL, grade TEXT NOT NULL, summary TEXT NOT NULL,
      tags JSONB NOT NULL DEFAULT '[]'::jsonb, status TEXT NOT NULL DEFAULT 'pending',
...
export async function getPrograms(): Promise<Program[]> {
  ...
  const rows = await sql`SELECT * FROM programs WHERE is_published = TRUE ORDER BY featured DESC, like_count DESC, created_at DESC`;
