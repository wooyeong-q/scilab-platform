...
const ANSWERS = ['7139', '6482', '3749', '4826'] as const;
const AUTO_HINTS = [
  '별의 색과 표면 온도를 비교해 가장 뜨거운 별부터 차례로 배열하세요.',
  '각 대원이 자기 기록에서 과학적으로 틀린 문장을 찾고, 그 문장 옆 점검번호를 역할 순서대로 전달하세요.',
  '네 대원이 맡은 주제에서 우리은하를 정확히 설명하는 선택지의 보안 숫자를 역할 순서대로 모으세요.',
  '가장 큰 공간에서 출발해 우리 집까지 범위를 줄여 보세요: 우주 → ? → ? → 지구.',
] as const;
...
export async function joinStarEscapeSession(code: string, nicknameValue: unknown, teamValue: unknown, roleValue: unknown) {
  ...
  await db`INSERT INTO star_escape_team_progress (session_id, team_name, stage_started_at)
    VALUES (${session.id}, ${team}, ${session.startedAt}) ON CONFLICT (session_id, team_name) DO NOTHING`;
  return {
...
export async function getStarEscapeState(code: string, playerId: string, playerKey: string) {
  ...
  const [sessionRows, progressRows, memberRows, leaderboardRows, hintRows] = await Promise.all([
    ...
    db`SELECT p.team_name, p.stage, p.completed_at, p.hint_count,
      ...
      ORDER BY (p.completed_at IS NOT NULL) DESC, p.stage DESC,
        ...
        p.team_name ASC`,
    db`SELECT id, team_name, message, stage, created_at FROM star_escape_hints
  ...
  return {
    ...
    teacherHints: hintRows.map((row) => ({ id: String(row.id), team: row.team_name ? String(row.team_name) : null, stage: Number(row.stage), message: String(row.message), createdAt: new Date(String(row.created_at)).toISOString() })).reverse(),
  };
}

export async function submitStarEscapeAnswer(code: string, playerId: string, playerKey: string, stageValue: unknown, answerValue: unknown) {
  await ensureStarEscapeDatabase();
  const player = await verifiedPlayer(code, playerId, playerKey);
  ...
  if (!Number.isInteger(stage) || stage < 1 || stage > 4 || answer.length !== 4) return { status: 'invalid' as const };
  const rows = await db`SELECT s.started_at, s.duration_seconds, p.stage, p.stage_started_at, p.penalty_seconds, p.completed_at
    FROM star_escape_team_progress p JOIN star_escape_sessions s ON s.id=p.session_id
  ...
  if (current.completed_at || Number(current.stage) !== stage) return { status: 'stale' as const };
  ...
  const correct = answer === ANSWERS[stage - 1];
  const stageStartedAt = current.stage_started_at ? new Date(String(current.stage_started_at)).getTime() : new Date(String(current.started_at)).getTime();
  const elapsedMs = Math.max(0, Math.min(3600000, Date.now() - stageStartedAt));
  await db`INSERT INTO star_escape_attempts (id, session_id, team_name, player_id, stage, answer, is_correct, elapsed_ms)
    VALUES (${randomUUID()}, ${sessionId}, ${team}, ${playerId}, ${stage}, ${answer}, ${correct}, ${elapsedMs})`;
  if (!correct) return { status: 'wrong' as const };
  const nextStage = stage + 1;
  const updated = await db`UPDATE star_escape_team_progress SET stage=${nextStage}, stage_started_at=NOW(),
      completed_at=CASE WHEN ${nextStage}=5 THEN NOW() ELSE completed_at END, updated_at=NOW()
    WHERE session_id=${sessionId} AND team_name=${team} AND stage=${stage} RETURNING stage, completed_at`;
  return updated[0] ? { status: 'correct' as const, stage: nextStage, completed: nextStage === 5 } : { status: 'stale' as const };
}

export async function requestStarEscapeHint(code: string, playerId: string, playerKey: string, stageValue: unknown) {
  await ensureStarEscapeDatabase();
  const player = await verifiedPlayer(code, playerId, playerKey);
  if (!player) return { status: 'unauthorized' as const };
  const stage = Number(stageValue);
  if (!Number.isInteger(stage) || stage < 1 || stage > 4) return { status: 'invalid' as const };
  const db = database();
  const sessionId = String(player.session_id);
  const team = String(player.team_name);
  const updated = await db`UPDATE star_escape_team_progress SET hint_count=hint_count+1,
    ...
    WHERE session_id=${sessionId} AND team_name=${team} AND stage=${stage}
  ...
  await db`INSERT INTO star_escape_hints (id, session_id, team_name, requester_id, hint_type, stage, message)
    VALUES (${randomUUID()}, ${sessionId}, ${team}, ${playerId}, 'request', ${stage}, ${`${String(player.nickname)} · ${stage}단계 힌트 요청`})`;
  return {
    status: 'ok' as const,
    hint: AUTO_HINTS[stage - 1],
    hintCount: count,
...
export async function getStarEscapeTeacherState(code: string, teacherKey: string) {
  ...
  const [teamRows, stageRows, playerRows, requestRows] = await Promise.all([
    db`SELECT p.team_name, p.stage, p.hint_count, p.penalty_seconds, p.completed_at,
        CASE WHEN s.started_at IS NULL THEN 0 ELSE EXTRACT(EPOCH FROM (COALESCE(p.completed_at, NOW())-s.started_at))+p.penalty_seconds END AS total_seconds
      FROM star_escape_team_progress p JOIN star_escape_sessions s ON s.id=p.session_id
      WHERE p.session_id=${session.id}
      ORDER BY (p.completed_at IS NOT NULL) DESC, p.stage DESC, total_seconds ASC, p.team_name`,
    db`SELECT stage, COUNT(*)::int AS attempts,
        COUNT(*) FILTER (WHERE is_correct)::int AS correct,
        COALESCE(AVG(elapsed_ms) FILTER (WHERE is_correct), 0)::float AS average_ms
      FROM star_escape_attempts WHERE session_id=${session.id} GROUP BY stage ORDER BY stage`,
    ...
    db`SELECT h.id, h.team_name, h.stage, h.message, h.created_at FROM star_escape_hints h
  ...
  return {
    ...
    stageStats: [1, 2, 3, 4].map((stage) => {
      const row = stageRows.find((item) => Number(item.stage) === stage) as Record<string, unknown> | undefined;
      const attempts = Number(row?.attempts || 0);
      const correct = Number(row?.correct || 0);
      return { stage, attempts, correct, accuracy: attempts ? Math.round(correct / attempts * 100) : 0, averageSeconds: Math.round(Number(row?.average_ms || 0) / 1000) };
    }),
    hintRequests: requestRows.map((row) => ({ id: String(row.id), team: String(row.team_name), stage: Number(row.stage), message: String(row.message), createdAt: new Date(String(row.created_at)).toISOString() })),
  };
}
...
export async function controlStarEscapeSession(code: string, teacherKey: string, input: Record<string, unknown>) {
  ...
  if (action === 'hint') {
    ...
    await db`INSERT INTO star_escape_hints (id, session_id, team_name, hint_type, stage, message)
      VALUES (${randomUUID()}, ${sessionId}, ${team}, 'teacher', ${stage}, ${message})`;
