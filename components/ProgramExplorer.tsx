...
export function ProgramExplorer({programs}:{programs:Program[]}){
  ...
  return <>
    <section className="searchPanel" aria-label="프로그램 검색과 분류"><label className="searchBox" htmlFor="program-search"><Search size={20}/><span className="srOnly">프로그램 검색</span><input id="program-search" type="search" value={query} onChange={(e)=>setQuery(e.target.value)} placeholder="프로그램명, 단원, 핵심어로 검색"/></label><div className="filters" aria-label="과학 영역">{categories.map((item)=><button type="button" key={item} className={category===item?'filter active':'filter'} aria-pressed={category===item} onClick={()=>setCategory(item)}>{item}</button>)}</div></section>
    ...
    {filtered.length===0?<section className="emptyState" role="status"><span>🧭</span><h3>조건에 맞는 프로그램이 없어요.</h3><p>검색어를 줄이거나 다른 과학 영역을 선택해 보세요.</p></section>:<section className="programGrid">{filtered.map((program)=><Link className="programCardLink" href={`/programs/${program.id}`} key={program.id} prefetch={false} onClick={()=>remember(program.id)}><article className="programCard">{program.thumbnailUrl?<img className="programThumb" src={program.thumbnailUrl} alt={`${program.title} 대표 이미지`} loading="lazy" decoding="async"/>:<div className="cardTop"><span className="programIcon" aria-hidden="true">{program.icon}</span>{program.featured&&<span className="featured"><Sparkles size={13}/> 추천</span>}</div>}<div className="cardBody"><span className="categoryLabel">{program.category}</span><h3>{program.title}</h3><p>{program.summary}</p></div><div className="tagRow">{program.tags.slice(0,3).map((tag)=><span key={tag}>{tag}</span>)}</div><div className="cardStats"><span aria-label={`조회 ${(program.viewCount||0).toLocaleString()}회`}><Eye size={14}/>{(program.viewCount||0).toLocaleString()}</span><span aria-label={`추천 ${(program.likeCount||0).toLocaleString()}개`}><Heart size={14}/>{(program.likeCount||0).toLocaleString()}</span></div><div className="cardFooter"><span>{program.grade}</span><span className="detailLink">자세히 보기 <ArrowUpRight size={16}/></span></div></article></Link>)}</section>}
  </>;
}
