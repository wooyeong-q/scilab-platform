...
export function ProgramExplorer({ programs }: { programs: Program[] }) {
  ...
  const categories = useMemo(
    () => ['전체', ...Array.from(new Set(programs.map((program) => program.category)))],
    [programs],
  );
  ...
  const filtered = useMemo(() => {
    ...
    return programs.filter((program) => {
      ...
      const text = [program.title, program.summary, program.category, program.grade, ...program.tags]
        .join(' ')
        .toLowerCase();
      return categoryMatch && text.includes(normalized);
    });
  }, [category, deferredQuery, programs]);
