...
import { LogOut, PencilLine, Save, Trash2, Check, X } from 'lucide-react';
...
export function AdminConsole() {
  ...
  async function load(){
    ...
    if(!response.ok){setError(data.error||'불러오지 못했습니다.');setAuthenticated(false);return;}
    setPrograms(data.programs);setSubmissions(data.submissions);setAuthenticated(true);
  }
  useEffect(()=>{load();},[]);
  ...
  async function save(e:React.FormEvent){
    ...
    const response=await fetch(`/api/admin/programs/${editing.id}`,{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});
    const data=await response.json().catch(()=>({}));
  ...
  return <>
    ...
    <section className="adminList">{programs.map((program)=><article className="adminCard" key={program.id}><div className="adminIcon">{program.icon}</div><div className="adminInfo"><span>{program.category} · {program.grade}</span><h2>{program.title}</h2><a href={program.url} target="_blank" rel="noreferrer">{program.url}</a></div><div className="adminActions"><button onClick={()=>startEdit(program)}><PencilLine size={16}/> 수정</button><button className="danger" onClick={()=>removeProgram(program.id)}><Trash2 size={16}/> 삭제</button></div></article>)}</section>
    {editing&&<div className="modalBackdrop" onMouseDown={()=>{setEditing(null);setTagInput('')}}><form className="editModal" onSubmit={save} onMouseDown={(e)=>e.stopPropagation()}><div className="modalHeader"><div><span>EDIT PROGRAM</span><h2>{editing.title}</h2></div><button type="button" onClick={()=>{setEditing(null);setTagInput('')}}>×</button></div><label>프로그램 이름<input value={editing.title} onChange={(e)=>setEditing({...editing,title:e.target.value})} required/></label><label>실행 주소<input type="url" value={editing.url} onChange={(e)=>setEditing({...editing,url:e.target.value})} required/></label><label>프로그램 설명<textarea value={editing.summary} onChange={(e)=>setEditing({...editing,summary:e.target.value,description:e.target.value})} required/></label><div className="formColumns"><label>과학 영역<input value={editing.category} onChange={(e)=>setEditing({...editing,category:e.target.value})}/></label><label>대상 학년<input value={editing.grade} onChange={(e)=>setEditing({...editing,grade:e.target.value})}/></label></div><label>핵심어<input value={tagInput} onChange={(e)=>setTagInput(e.target.value)} placeholder="예: 광물, 굳기, 조흔색"/><small style={{color:'#667085',fontWeight:500}}>쉼표로 구분하면 여러 개를 등록할 수 있습니다.</small></label><button className="saveButton" type="submit"><Save size={17}/> 저장</button></form></div>}
  </>;
}
