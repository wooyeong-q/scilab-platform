...
export function AdminConsole() {
  ...
  return <>
    ...
    <section className="adminList">{programs.map((program)=><article className="adminCard" key={program.id}><div className="adminIcon">{program.icon}</div><div className="adminInfo"><span>{program.category} · {program.grade}</span><h2>{program.title}</h2><a href={program.url} target="_blank" rel="noreferrer">{program.url}</a></div><div className="adminActions"><button onClick={()=>setEditing(program)}><PencilLine size={16}/> 수정</button><button className="danger" onClick={()=>removeProgram(program.id)}><Trash2 size={16}/> 삭제</button></div></article>)}</section>
    {editing&&<div className="modalBackdrop" onMouseDown={()=>setEditing(null)}><form className="editModal" onSubmit={save} onMouseDown={(e)=>e.stopPropagation()}><div className="modalHeader"><div><span>EDIT PROGRAM</span><h2>{editing.title}</h2></div><button type="button" onClick={()=>setEditing(null)}>×</button></div><label>실행 주소<input type="url" value={editing.url} onChange={(e)=>setEditing({...editing,url:e.target.value})} required/></label><label>프로그램 설명<textarea value={editing.summary} onChange={(e)=>setEditing({...editing,summary:e.target.value,description:e.target.value})} required/></label><div className="formColumns"><label>과학 영역<input value={editing.category} onChange={(e)=>setEditing({...editing,category:e.target.value})}/></label><label>대상 학년<input value={editing.grade} onChange={(e)=>setEditing({...editing,grade:e.target.value})}/></label></div><label>핵심어<input value={editing.tags.join(', ')} onChange={(e)=>setEditing({...editing,tags:e.target.value.split(',').map(t=>t.trim()).filter(Boolean)})}/></label><button className="saveButton" type="submit"><Save size={17}/> 저장</button></form></div>}
  </>;
}
