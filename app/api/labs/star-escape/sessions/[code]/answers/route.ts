...
export async function POST(request: Request, { params }: Context) {
  try {
    ...
    const result = await submitStarEscapeAnswer(code, request.headers.get('x-player-id') || '', request.headers.get('x-player-key') || '', body.stage, body.answer);
    if (result.status === 'unauthorized') return noStore({ error: '입장 정보를 확인해 주세요.' }, { status: 401 });
    if (result.status === 'invalid') return noStore({ error: '4자리 보안번호를 입력하세요.' }, { status: 400 });
