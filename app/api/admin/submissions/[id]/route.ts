...
export async function PATCH(request:Request,{params}:{params:Promise<{id:string}>}){
  ...
  const{id}=await params;const{action}=await request.json();await ensureDatabase();
  if(action==='approve'){
    const rows=await sql!`SELECT * FROM submissions WHERE id=${id} LIMIT 1`;const item=rows[0];
...
export async function DELETE(_request:Request,{params}:{params:Promise<{id:string}>}){if(!(await isAdmin()))return NextResponse.json({error:'인증이 필요합니다.'},{status:401});const{id}=await params;await ensureDatabase();await sql!`DELETE FROM submissions WHERE id=${id}`;return NextResponse.json({ok:true});}
