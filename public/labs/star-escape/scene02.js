...
function restore(){try{visited=new Set(JSON.parse(localStorage.getItem(storeKey('visited'))||'[]'));foundCards=new Set(JSON.parse(localStorage.getItem(storeKey('cards'))||'[]'));definitionFound=localStorage.getItem(storeKey('definition'))==='found';introStep=localStorage.getItem(storeKey('intro'))==='done'?intro.length:0}catch(e){visited=new Set();foundCards=new Set();definitionFound=false;introStep=0}}
function save(){try{localStorage.setItem(storeKey('visited'),JSON.stringify(Array.from(visited)));localStorage.setItem(storeKey('cards'),JSON.stringify(Array.from(foundCards)));if(definitionFound)localStorage.setItem(storeKey('definition'),'found')}catch(e){}}
...
if(q===2&&id==='hatch'){definitionFound=true;save();inspect={id:'hatch',secretDefinition:true,name:'바닥 점검구 안쪽의 기록',text:'별을 (□)개월 간격으로 관(□)했을 때, 두 관측 방향 사이 각의 (□)/(□)이 연주시차이다.'};draw();return}
...
function objective(q){if(q===1)return'관측 카드로 <b>위치 변화가 가장 큰 별</b>을 찾으세요.';if(q===2)return'수수께끼가 가리키는 기록을 찾아 <b>분석 장치 암호</b>를 푸세요.';return'별의 위치 변화 크기로 <b>거리 순서</b>를 복원하세요.'}
...
function roomMarkup(q){var required=q===1?'calibration':q===2?'orbit':'distance',stage=q-1;var hotspots=objects.map(function(o){var found=cardForObject(o.id),done=found&&foundCards.has(found.card.id),isSecret=q===2&&o.id==='hatch';var cls='s2-hotspot'+(done?' visited':'')+(!done&&found?' card-clue':'')+(isSecret?' secret-clue':'')+(o.id===required?' required':'');var label=isSecret?'수수께끼가 가리키는 숨은 기록':found?'숨겨진 '+found.card.epoch+' 관측 카드':o.name;return'<button class="'+cls+'" data-s2-object="'+o.id+'" aria-label="'+esc(label)+'" style="left:'+o.box[0]+'%;top:'+o.box[1]+'%;width:'+o.box[2]+'%;height:'+o.box[3]+'%"></button>'}).join('');
...
function definitionRecord(){return definitionFound?'<div class="s2-definition-record"><small>바닥 점검구에서 찾은 정의 기록</small><p>별을 <strong>( □ )개월</strong> 간격으로 관<strong>( □ )</strong>했을 때,<br>두 관측 방향 사이 각의 <strong>( □ ) / ( □ )</strong>이 연주시차이다.</p><span>빈칸에 들어갈 내용을 순서대로 이어 쓰세요.</span></div>':'<div class="s2-definition-locked"><b>정의 기록을 아직 찾지 못했습니다.</b><span>수수께끼가 가리키는 방 안의 장소를 직접 눌러 확인하세요.</span><em>추가 도움: 벽이 아니라 발밑을 확인하세요.</em></div>'}
function puzzle2(){return'<div class="s2-panel s2-q2-panel">'+guideBlock(2,'ANNUAL PARALLAX ANALYZER','연주시차 분석 장치 암호 해제','수수께끼가 가리키는 장소에서 연주시차 정의 기록을 찾고, 빈칸의 내용을 순서대로 이어 비밀번호로 입력하세요.')+'<div class="s2-riddle-box"><small>위치 수수께끼</small><p>나는 문이지만 벽에 서 있지 않는다.<br>열려도 누구도 다음 방으로 가지 않는다.<br>모든 장치가 나를 둘러싸고 있지만,<br>아무도 나를 올려다보지 않는다.<br><b>나는 무엇인가?</b></p></div>'+definitionRecord()+'<label class="s2-password-field"><span>분석 장치 비밀번호</span><input id="s2Password" type="text" maxlength="5" autocomplete="off" autocapitalize="off" spellcheck="false" placeholder="빈칸 내용을 이어 입력"></label></div>'}
