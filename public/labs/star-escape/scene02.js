...
var visited=new Set(),selectedCard='',slotValue='',recordSlots=['',''],q2Step=1,halfValue=0,halfDirection='right',distanceSlots=['','','',''];
var intro=[
...
function overlayObjects(q,endingState){var distanceImage=endingState?'scene2_distance_console_success.png':q>2?'scene2_distance_console_on.png':'scene2_distance_console_off.png';var panelImage=endingState==='open'?'scene2_hidden_panel_open.png':endingState?'scene2_hidden_panel_unlocked.png':'scene2_hidden_panel_closed.png';return'<img class="s2-status-object calibration" src="'+img(q>1?'scene2_calibration_console_success.png':'scene2_calibration_console_idle.png')+'" alt="">'+
    '<img class="s2-status-object orbit" src="'+img(q>1?'scene2_orbit_projector_on.png':'scene2_orbit_projector_off.png')+'" alt="">'+
    '<img class="s2-status-object distance" src="'+img(distanceImage)+'" alt="">'+
    '<img class="s2-status-object wall-panel" src="'+img(panelImage)+'" alt="">'}
...
function q2IntelMarkup(intel){return'<div class="s2-role-intel"><b>'+esc(intel.title)+'</b><p>'+esc(intel.text)+'</p>'+(viewRole===4?'<div class="s2-manual-diagram" aria-label="전체 시차와 연주시차의 관계"><i class="start"></i><i class="middle"></i><i class="end"></i><span class="whole">전체 시차 2p</span><span class="half">연주시차 p</span></div>':'')+'</div>'}
function puzzle2(){var intel=q2Intel[viewRole-1];if(q2Step===2)return'<div class="s2-panel"><div class="s2-guide"><small>ANNUAL PARALLAX CALIBRATION · STEP 2/2</small><b>연주시차에 해당하는 범위를 표시하세요.</b><p>양쪽 관측 위치 사이 전체 시차 중 절반에 해당하는 범위를 드래그해 표시하세요.</p></div>'+roleTabs()+q2IntelMarkup(intel)+'<div class="s2-measure"><img src="'+img('scene2_record_AB_compare.png')+'" alt="3월과 9월의 동일 배경 별 관측 비교"><span class="s2-observation-label left">3월 18일 · 관측 A</span><span class="s2-observation-label right">9월 18일 · 관측 B</span><div class="s2-midline"><span>기준 위치</span></div><div class="s2-measure-line"></div><div class="s2-half-arrow '+halfDirection+'" style="--half:'+halfValue+'"></div></div><div class="s2-measure-control"><div class="s2-direction"><button data-s2-direction="left" class="'+(halfDirection==='left'?'on':'')+'">A에서 가운데까지</button><button data-s2-direction="right" class="'+(halfDirection==='right'?'on':'')+'">B에서 가운데까지</button></div><input id="s2HalfRange" type="range" min="0" max="100" value="'+halfValue+'" aria-label="전체 시차 중 표시할 비율"></div></div>';
...
if(q===2&&q2Step===2){document.querySelectorAll('[data-s2-direction]').forEach(function(b){b.onclick=function(){halfDirection=b.dataset.s2Direction;draw()}});var range=document.getElementById('s2HalfRange');range.oninput=function(){halfValue=Number(range.value);var a=document.querySelector('.s2-half-arrow');if(a)a.style.setProperty('--half',halfValue)}}
...
if(q===2&&q2Step===1){if(recordSlots.filter(Boolean).length<2)return bad('관측 기록 두 장을 모두 배치하세요.');if(recordSlots.slice().sort().join('')!=='AB')return bad('두 관측 기록의 날짜 간격을 다시 확인하세요.');q2Step=2;halfValue=0;halfDirection='right';save();draw();return}
...
];function paint(){if(step===0){game.innerHTML='<div class="s2-shell"><div class="s2-room">'+overlayObjects(3,'unlocked')+'<div class="s2-title"><small>DISTANCE CALIBRATION ROOM 02</small><b>별 거리 자료 복구 완료</b></div><div class="s2-system">별 거리 자료 복구 완료 · 벽면 패널 잠금 해제</div><div class="s2-objective">현재 목표 · 노란색으로 빛나는 <b>벽면 패널</b>을 조사하세요.</div><button class="s2-hotspot required" id="s2EndPanel" aria-label="잠금 해제된 벽면 패널" style="left:88%;top:12%;width:11%;height:35%"></button></div></div>';document.getElementById('s2EndPanel').onclick=function(){step=1;paint()};return}
if(step===1){game.innerHTML='<div class="s2-shell"><div class="s2-room">'+overlayObjects(3,'open')+'<div class="s2-ending"><div class="s2-ending-card"><small>HIDDEN CHANNEL FOUND</small><h2>숨겨진 수동 송신기</h2><img class="s2-ending-image" src="'+img('scene2_hidden_transmitter.png')+'" alt="벽면 패널 안에 숨겨진 수동 송신기"><p>정거장의 공식 장비와 다른 작은 송신 장치가 벽 안에 숨겨져 있습니다.</p><button class="primary" id="s2PlaySignal">송신 기록 재생</button></div></div></div></div>';document.getElementById('s2PlaySignal').onclick=function(){step=2;line=0;paint()};return}
if(step===2){var d=signal[line];game.innerHTML='<div class="s2-shell"><div class="s2-room">'+overlayObjects(3,'open')+'<div class="s2-ending s2-signal-stage"><img class="s2-transmitter-live" src="'+img('scene2_hidden_transmitter_on.png')+'" alt="작동 중인 수동 송신기"><button class="s2-ending-dialogue" id="s2SignalNext"><small>'+esc(d[0])+'</small><p>'+esc(d[1])+'</p><i>'+(line===signal.length-1?'3번 구획 확인':'터치하여 계속')+' ▼</i></button></div></div></div>';document.getElementById('s2SignalNext').onclick=function(){line++;if(line>=signal.length){step=3;line=0}paint()};return}
game.innerHTML='<div class="s2-shell"><div class="s2-room">'+overlayObjects(3,'open')+'<img class="s2-door-open-overlay" src="'+img('scene2_door3_open_overlay.png')+'" alt="열린 3번 구획 문"><div class="s2-ending"><div class="s2-ending-card"><small>SCENE 02 · COMPLETE</small><h2>03 — 별빛 분석 구획</h2><img class="s2-corridor-image" src="'+img('scene2_corridor3_inside.png')+'" alt="3번 구획으로 이어지는 복도"><p>거리 보정 시스템이 정상화되어 다음 구획의 문이 열렸습니다.</p><button class="primary" id="s2Continue">3번 구획으로 이동</button></div></div></div></div>';document.getElementById('s2Continue').onclick=onContinue}paint()}
