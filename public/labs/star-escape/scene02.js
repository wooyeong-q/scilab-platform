...
var visited=new Set(),foundCards=new Set(),selectedCard='',selectionKind='',slotValue='',parallaxChoice='',loadedPairRole=0,distanceSlots=['','','',''];
var intro=[
...
{role:3,star:'C',shift:44,level:'큼',cards:[{id:'C-mar',object:'hatch',epoch:'3월 18일',source:'바닥 점검구'},{id:'C-sep',object:'storage',epoch:'9월 18일',source:'관측 필름 보관함'}]},
...
function restore(){try{visited=new Set(JSON.parse(localStorage.getItem(storeKey('visited'))||'[]'));foundCards=new Set(JSON.parse(localStorage.getItem(storeKey('cards'))||'[]'));introStep=localStorage.getItem(storeKey('intro'))==='done'?intro.length:0}catch(e){visited=new Set();foundCards=new Set();introStep=0}}
function save(){try{localStorage.setItem(storeKey('visited'),JSON.stringify(Array.from(visited)));localStorage.setItem(storeKey('cards'),JSON.stringify(Array.from(foundCards)))}catch(e){}}
...
if((q===1&&id==='calibration')||(q===2&&id==='orbit')||(q===3&&id==='distance')){if(q===1&&foundCount(ctx.state.player.role)<2){inspect={id:id,name:'관측 센서 점검 장치',text:'먼저 방 안에서 내 별의 3월·9월 관측 카드 두 장을 찾아 단서 탭에 저장하세요.'};draw();return}inspect=null;puzzleOpen=true;clueTabOpen=false;draw();return}
...
function objective(q){if(q===1)return'내 별의 관측 카드 <b>2장</b>을 찾고 가장 큰 위치 변화를 고르세요.';if(q===2)return'단서 탭에서 두 카드를 꺼내 <b>연주시차 p</b>를 복구하세요.';return'같은 네 별의 연주시차를 비교해 <b>거리표</b>를 복원하세요.'}
function missionChain(q){var labels=['1. 카드 수집·센서 보정','2. 연주시차 복구','3. 같은 별 거리표 복원'];return'<div class="s2-mission-chain" aria-label="거리 자료 복구 절차">'+labels.map(function(label,i){var step=i+1;return'<span class="'+(step<q?'done':step===q?'active':'')+'">'+label+'</span>'+(step<3?'<i>→</i>':'')}).join('')+'</div>'}
function storyWhy(q){if(q===1)return'각 요원이 맡은 별의 3월·9월 사진을 찾아야 겉보기 위치 변화가 가장 큰 기준별을 정할 수 있습니다.';if(q===2)return'저장한 두 사진은 6개월 간격 관측입니다. 두 겉보기 위치의 전체 각거리 2p에서 연주시차 p를 복구해야 합니다.';return'처음부터 조사한 별 A~D를 다시 비교합니다. 연주시차가 클수록 가까우므로 거리 순서를 항법 장치에 입력할 수 있습니다.'}
function roomMarkup(q){var required=q===1?'calibration':q===2?'orbit':'distance';var hotspots=objects.map(function(o){var found=cardForObject(o.id),done=found&&foundCards.has(found.card.id);var cls='s2-hotspot'+(done?' visited':'')+(o.id===required?' required':'');var label=found?'숨겨진 '+found.card.epoch+' 관측 카드':o.name;return'<button class="'+cls+'" data-s2-object="'+o.id+'" aria-label="'+esc(label)+'" style="left:'+o.box[0]+'%;top:'+o.box[1]+'%;width:'+o.box[2]+'%;height:'+o.box[3]+'%"></button>'}).join('');
return'<div class="s2-shell"><div class="s2-room s2-room-q'+q+'"><div class="s2-title"><small>DISTANCE CALIBRATION ROOM 02</small><b>어긋난 별의 위치</b></div><div class="s2-objective">현재 목표 · '+objective(q)+'</div>'+missionChain(q)+hotspots+(!puzzleOpen?clueTabButton():'')+(banner?'<div class="s2-system">'+esc(banner)+'</div>':'')+(transitionNotice?transitionMarkup():'')+(inspect?inspectMarkup():'')+(introStep<intro.length?dialogueMarkup():'')+(puzzleOpen?puzzleMarkup(q):'')+(clueTabOpen?clueTabMarkup(q):'')+'</div></div>'}
...
function clueTabMarkup(q){var tabs=roleTabs(),set=setForRole(viewRole),count=foundCount(viewRole),ready=count===2;return'<section class="s2-clue-drawer" role="dialog" aria-modal="true" aria-label="관측 카드 단서 탭"><header><div><small>EVIDENCE ARCHIVE</small><h2>관측 카드 단서 탭</h2></div><button id="s2ClueClose" aria-label="단서 탭 닫기">×</button></header><div class="s2-clue-body">'+tabs+'<div class="s2-clue-summary"><b>대원 '+viewRole+' · 담당 별 '+set.star+'</b><span>'+count+'/2장 발견</span></div>'+pairMarkup(set,'drawer')+'<p class="s2-clue-help">두 카드의 배경별은 완전히 같습니다. <b>파란 표적별이 3월에서 9월 사이 얼마나 옮겨 보였는지</b> 비교하세요.</p>'+(ready&&q>=2?'<button class="primary s2-load-pair" data-s2-load-pair="'+viewRole+'">두 장을 문제 비교대에 꺼내기</button>':!ready?'<div class="s2-clue-empty">방 안의 파란 점을 더 조사해 나머지 카드를 찾으세요.</div>':'')+'</div></section>'}
function inspectMarkup(){if(inspect.card)return'<section class="s2-inspect s2-card-found"><div class="s2-inspect-media">'+observationCard(inspect.set,inspect.card,'compact')+'</div><div><small>대원 '+inspect.role+' · 별 '+inspect.set.star+'</small><b>'+esc(inspect.card.epoch)+' 관측 카드 발견</b><span class="s2-device-state">단서 탭에 자동 저장됨 · '+foundCount(inspect.role)+'/2</span><p>다른 날짜 카드와 배경별은 같습니다. 두 장을 모두 찾은 뒤 파란 표적별의 위치만 비교하세요.</p></div><button class="s2-inspect-close" id="s2InspectClose">×</button></section>';
...
function puzzle1(){return'<div class="s2-panel">'+guideBlock(1,'PARALLAX SENSOR CALIBRATION','겉보기 위치 변화가 가장 큰 별 선택','각 요원은 단서 탭에서 자기 별의 3월·9월 카드를 비교하고, 변화 크기를 서로 말해 주세요. 가장 크게 이동해 보인 별을 기준 슬롯에 넣으세요.')+'<div class="s2-coop-note">내 화면에는 내 별의 카드만 저장됩니다. 네 요원이 <b>“매우 큼 / 큼 / 보통 / 매우 작음”</b>처럼 변화 크기를 공유해야 합니다.</div><div class="s2-drag-area"><div><div class="s2-bank">'+['A','B','C','D'].map(function(x){return card(x,slotValue===x,'bank')}).join('')+'</div>'+selectionNotice('card')+'</div><div class="s2-slot-wrap"><span class="s2-slot-label">기준별 슬롯</span><div class="s2-slot '+(slotValue?'filled':'')+'" data-s2-slot="single">'+(slotValue?card(slotValue,false,'slot'):'선택한 별을<br>여기에 놓으세요.')+'</div></div></div></div>'}
function parallaxBoard(){if(!loadedPairRole)return'<div class="s2-compare-empty"><b>비교대가 비어 있습니다.</b><span>위의 단서 탭을 열고 3월·9월 카드 두 장을 꺼내세요.</span><button class="secondary" id="s2OpenClues">단서 탭 열기</button></div>';var set=setForRole(loadedPairRole),left=50-set.shift/2,right=50+set.shift/2;return'<div class="s2-loaded-pair"><div class="s2-loaded-title"><b>대원 '+loadedPairRole+' · 별 '+set.star+' 관측 카드 비교</b><button id="s2ChangePair">다른 카드 꺼내기</button></div>'+pairMarkup(set,'compare')+'</div><div class="s2-parallax-board '+(parallaxChoice?'choice-'+parallaxChoice:'')+'"><div class="s2-parallax-field" aria-label="같은 배경별 위에 겹친 3월과 9월 표적 위치"><span class="s2-target-pos pos-a" style="left:'+left+'%"><i></i><b>3월 위치</b></span><span class="s2-target-pos pos-o"><i></i><b>O</b><small>태양 기준 방향</small></span><span class="s2-target-pos pos-b" style="left:'+right+'%"><i></i><b>9월 위치</b></span><span class="s2-whole-segment" style="left:'+left+'%;width:'+set.shift+'%">두 겉보기 위치 사이 = 2p</span><span class="s2-half-segment" style="left:'+left+'%;width:'+(set.shift/2)+'%">한쪽에서 기준까지</span></div><p class="s2-science-note">두 카드의 배경별을 맞춰 겹친 모형입니다. O는 별의 실제 위치가 아니라 태양에서 본 기준 방향입니다.</p><div class="s2-parallax-options"><button data-s2-parallax="whole" class="'+(parallaxChoice==='whole'?'on':'')+'"><b>3월 위치 ↔ 9월 위치</b><span>두 관측 위치 사이 전체 각거리</span></button><button data-s2-parallax="half" class="'+(parallaxChoice==='half'?'on':'')+'"><b>한 관측 위치 ↔ O</b><span>연주시차 p에 해당하는 구간</span></button></div></div>'}
function puzzle2(){return'<div class="s2-panel">'+guideBlock(2,'ANNUAL PARALLAX CALIBRATION','관측 카드로 연주시차 p 복구','단서 탭에서 실제로 발견한 3월·9월 카드 두 장을 비교대에 꺼내세요. 두 겉보기 위치 사이 전체 각거리는 2p입니다.')+parallaxBoard()+'</div>'}
...
function puzzleMarkup(q){return'<section class="s2-puzzle"><header class="s2-puzzle-head"><div><small>SCENE 02 · CALIBRATION '+q+'/3</small><h2>'+(q===1?'관측 센서 점검':q===2?'관측 카드 연주시차 비교':'별 거리 자료 복구')+'</h2></div><div class="s2-puzzle-actions">'+clueTabButton('inside')+'<button class="s2-close" id="s2PuzzleClose">×</button></div></header><div class="s2-puzzle-body">'+(q===1?puzzle1():q===2?puzzle2():puzzle3())+'<div class="hintbox" id="hintbox"></div></div><footer class="s2-footer"><p class="s2-feedback" id="feedback">'+(q===1?'단서 탭의 두 카드를 비교한 뒤 기준별을 선택하세요.':q===2?'단서 카드 두 장을 비교대에 꺼내 연주시차 p를 선택하세요.':'네 요원의 카드 변화를 공유하고 가까운 순서로 배치하세요.')+'</p><button class="secondary" id="hintBtn">힌트 · '+Math.min(ctx.state.progress.hintCount,3)+'/3 무료</button><button class="primary" id="s2Submit">'+(q===1?'보정 실행':q===2?'연주시차 복구':'거리 자료 복구')+'</button></footer></section>'}
...
bindPlacement(q);
if(q===2)document.querySelectorAll('[data-s2-parallax]').forEach(function(b){b.onclick=function(){parallaxChoice=b.dataset.s2Parallax;draw()}});
...
if(q===2){if(!loadedPairRole)return bad('먼저 단서 탭에서 3월·9월 카드 두 장을 비교대에 꺼내세요.');if(!parallaxChoice)return bad('두 구간 중 연주시차 p에 해당하는 구간을 선택하세요.');if(parallaxChoice!=='half')return bad('3월 위치와 9월 위치 사이는 전체 각거리 2p입니다. 연주시차 p는 그 절반입니다.');await ctx.submit('ABP',this);return}
...
function bindClueControls(){var tab=document.getElementById('s2ClueTab'),inside=document.getElementById('s2OpenClues'),change=document.getElementById('s2ChangePair');function open(){clueTabOpen=true;viewRole=ctx.state.player.role;draw()}if(tab)tab.onclick=open;if(inside)inside.onclick=open;if(change)change.onclick=open;var close=document.getElementById('s2ClueClose');if(close)close.onclick=function(){clueTabOpen=false;draw()};document.querySelectorAll('[data-s2-role]').forEach(function(b){b.onclick=function(){viewRole=Number(b.dataset.s2Role);draw()}});document.querySelectorAll('[data-s2-load-pair]').forEach(function(b){b.onclick=function(){loadedPairRole=Number(b.dataset.s2LoadPair);parallaxChoice='';clueTabOpen=false;draw()}})}
...
function render(options){ctx=options;var next=options.state.session.code+':'+options.state.player.team+':'+options.state.player.role;if(identity!==next){identity=next;restore();inspect=null;puzzleOpen=false;clueTabOpen=false;lastQuestion=0;transitionNotice=null;viewRole=options.state.player.role}var q=question();if(lastQuestion&&q!==lastQuestion){inspect=null;puzzleOpen=false;clueTabOpen=false;clearSelection();slotValue='';distanceSlots=['','','',''];parallaxChoice='';loadedPairRole=0;if(q===2){banner='관측 센서 보정 완료 · 궤도 관측 투영기 연결';transitionNotice={title:'1단계 완료 · 기준별 선택',text:'네 요원의 관측 카드에서 별 A의 위치 변화가 가장 큰 것을 확인했습니다. 이제 단서 탭에서 실제 3월·9월 카드를 꺼내 전체 각거리 2p와 연주시차 p의 관계를 복구하세요.'}}else if(q===3){banner='연주시차 복구 완료 · 거리 자료 복구 장치 활성화';transitionNotice={title:'2단계 완료 · 연주시차 복구',text:'카드 두 장으로 연주시차 p의 의미를 복원했습니다. 같은 별 A~D의 위치 변화 크기를 다시 공유해 가까운 순서로 거리표를 완성하세요.'}}}lastQuestion=q;draw()}
