...
var ctx=null,identity='',introStep=0,inspect=null,puzzleOpen=false,clueTabOpen=false,banner='',lastQuestion=0,viewRole=1,transitionNotice=null,endingStep=0,endingLine=0,endingIdentity='',endingDoorPlayed=false;
...
{id:'pencil',name:'콘솔 위 연필',box:[29,48,6,10]},
...
function play(name){if(ctx&&ctx.sound&&typeof ctx.sound.play==='function')ctx.sound.play(name)}
...
document.querySelectorAll('[data-s2-slot]').forEach(function(slot){slot.onclick=function(){if(selectionKind!=='card'||!selectedCard)return;slotValue=selectedCard;clearSelection();play('insert');draw()}});
document.querySelectorAll('[data-s2-distance-slot]').forEach(function(slot){slot.onclick=function(){if(selectionKind!=='star'||!selectedCard)return;var target=Number(slot.dataset.s2DistanceSlot),value=selectedCard;distanceSlots=distanceSlots.map(function(x){return x===value?'':x});distanceSlots[target]=value;clearSelection();play('insert');draw()}});
...
function draw(){if(!ctx||!ctx.game)return;var q=question();ctx.game.innerHTML=roomMarkup(q);bindRoom();if(puzzleOpen)bindPuzzle(q);if(banner)setTimeout(function(){banner='';var e=document.querySelector('.s2-system');if(e)e.remove()},6000)}
...
function renderEnding(game,onContinue){var runIdentity=identity||'ending';if(endingIdentity!==runIdentity){endingIdentity=runIdentity;endingStep=0;endingLine=0;endingDoorPlayed=false;play('cabinetOpen12')}var signal=[
['미확인 음성','…여기까지 왔다면…'],
['미확인 음성','거리 자료는 복구했겠지.'],
