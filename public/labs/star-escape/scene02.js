...
function dialogueMarkup(){var d=intro[Math.min(introStep,intro.length-1)];return'<button class="s2-dialogue" id="s2Dialogue"><img src="'+speakerPortrait(d[0])+'" alt="'+esc(d[0])+'"><span><small>'+esc(d[0])+'</small><p>'+esc(d[1])+'</p></span><i class="advance">터치하여 계속 ▼</i></button>'}
...
];function paint(){if(endingStep===0){game.innerHTML='<div class="s2-shell"><div class="s2-room s2-room-stage3 s2-room-recovered"><div class="s2-title"><small>DISTANCE CALIBRATION ROOM 02</small><b>별 거리 자료 복구 완료</b></div><div class="s2-recovery-flash"><small>SYSTEM RECOVERY COMPLETE</small><b>거리표 복구 · 벽면 패널 잠금 해제</b><span>오른쪽 벽면 패널 표시등이 파란색으로 바뀌었습니다.</span></div><div class="s2-objective">현재 목표 · 잠금이 풀린 <b>벽면 패널</b>을 조사하세요.</div><img class="s2-panel-unlocked-overlay" src="'+img('scene2_hidden_panel_unlocked.png')+'" alt=""><button class="s2-hotspot required s2-ending-panel-hotspot" id="s2EndPanel" data-label="잠금 해제된 벽면 패널" aria-label="잠금 해제된 벽면 패널" style="left:88%;top:8%;width:12%;height:39%"></button></div></div>';document.getElementById('s2EndPanel').onclick=function(){endingStep=1;paint()};return}
