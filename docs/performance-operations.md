...
- `npm run db:migrate` is an explicit operator command. Supply `DATABASE_URL` in
  the process environment. It applies only the existing `CREATE ... IF NOT EXISTS`
  and `ADD COLUMN IF NOT EXISTS` statements in one transaction.
